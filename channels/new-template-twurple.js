async function main() {
  ...
  chatClient.onDisconnect(async (manually, reason) => {
    if (manually) {
      ...
      console.log(`[${getTimestamp()}] warning: Disconnected from chat unexpectedly. Reason: ${reason || 'Unknown'}`);
      
      // Attempt reconnection after unexpected disconnection
      try {
        console.log(`[${getTimestamp()}] info: Attempting to reconnect to chat...`);
        await sleep(5000); // Wait 5 seconds before attempting reconnection
        await chatClient.reconnect();
        console.log(`[${getTimestamp()}] info: Successfully reconnected to chat`);
      } catch (reconnectError) {
        console.log(`[${getTimestamp()}] error: Failed to reconnect to chat: ${reconnectError.message}`);
        // Don't exit here, let Twurple handle its own reconnection attempts
      }
    }
  });
  ...
  // Update activity timestamp when we receive messages
  chatClient.onMessage(() => {
    lastActivityTime = Date.now();
  });

  // Update activity timestamp on other chat events
  chatClient.onConnect(() => {
    lastActivityTime = Date.now();
  });

  // Health check function with ghost connection detection
  ...
  function checkConnectionHealth() {
    ...
    const maxGhostTime = 30 * 60 * 1000; // 30 minutes - force reconnect

    if (timeSinceLastActivity > maxInactivityTime) {
      console.log(`[${getTimestamp()}] warning: No chat activity for ${Math.floor(timeSinceLastActivity / 1000 / 60)} minutes - connection may be stale`);
      
      // If no activity for over 30 minutes, assume ghost connection and force reconnect
      if (timeSinceLastActivity > maxGhostTime) {
        console.log(`[${getTimestamp()}] CRITICAL: Suspected ghost connection - forcing reconnection after ${Math.floor(timeSinceLastActivity / 1000 / 60)} minutes of inactivity`);
        
        try {
          // Force disconnect and reconnect
          chatClient.quit().then(async () => {
            console.log(`[${getTimestamp()}] info: Disconnected from ghost connection, attempting fresh connection...`);
            await sleep(5000); // Wait 5 seconds
            await chatClient.connect();
            await chatClient.join(channelName);
            console.log(`[${getTimestamp()}] info: Successfully reconnected after ghost connection detection`);
            lastActivityTime = Date.now(); // Reset activity timer
          }).catch(async (error) => {
            console.log(`[${getTimestamp()}] error: Failed to recover from ghost connection: ${error.message}`);
            console.log(`[${getTimestamp()}] FATAL: Restarting bot to recover from ghost connection`);
            process.exit(1); // Let PM2 restart the bot
          });
        } catch (error) {
          console.log(`[${getTimestamp()}] FATAL: Ghost connection recovery failed: ${error.message}`);
          process.exit(1); // Let PM2 restart the bot
        }
        return; // Skip the rest of the health check
      }

      // Try to send a quiet command to test the connection (for shorter inactivity periods)
      try {
        chatClient.say(channelName, '/mods').catch(error => {
          console.log(`[${getTimestamp()}] warning: Connection test failed: ${error.message}`);
          healthCheckFailures++;
        });
      } catch (error) {
        console.log(`[${getTimestamp()}] warning: Unable to test connection: ${error.message}`);
        healthCheckFailures++;
      }

      // If health checks keep failing, also force reconnection
      if (healthCheckFailures >= 3) {
        console.log(`[${getTimestamp()}] CRITICAL: Multiple health check failures (${healthCheckFailures}) - forcing reconnection`);
        ...
        try {
          chatClient.quit().then(async () => {
            ...
            await chatClient.connect();
            await chatClient.join(channelName);
            console.log(`[${getTimestamp()}] info: Successfully reconnected after health check failures`);
            lastActivityTime = Date.now();
          }).catch(() => {
            console.log(`[${getTimestamp()}] FATAL: Health check recovery failed, restarting bot`);
            process.exit(1);
          });
        } catch (error) {
          console.log(`[${getTimestamp()}] FATAL: Health check recovery error: ${error.message}`);
          process.exit(1);
        }
      }
    } else {
  ...
  // Start connection health monitoring (check every 5 minutes)
  connectionHealthInterval = setInterval(checkConnectionHealth, 5 * 60 * 1000);
...
// Enhanced error handling with detailed logging and restart mechanism
main().catch(error => {
